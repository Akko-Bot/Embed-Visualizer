...
const fromAkkoToNadekoEmbed = (embed) => {
  ...
  if (embed.header) {
    if (embed.header.thumbnail_url) { result.thumbnail = embed.header.thumbnail_url }

    if (embed.header.author) {
      if (embed.header.author.text) { result.author.name = embed.header.author.text }

      if (embed.header.author.url) { result.author.url = embed.header.author.url }

      if (embed.header.author.image_url) { result.author.icon_url = embed.header.author.image_url }
    }
  }

  if (embed.body) {
    if (embed.body.description) { result.description = embed.body.description }

    if (embed.body.image_url) { result.image = embed.body.image_url }

    if (embed.body.title) {
      if (embed.body.title.text) { result.title = embed.body.title.text }

      if (embed.body.title.url) { result.url = embed.body.title.url }
    }
  }
  ...
  if (embed.footer) {
    ...
    if (embed.footer.image_url) { result.footer.icon_url = embed.footer.image_url }
  }
