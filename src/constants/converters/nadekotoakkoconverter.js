...
const fromNadekoToAkkoEmbed = (embed) => {
    ...
    if (embed.thumbnail)
        result.header.thumbnail_url = embed.thumbnail

    if (embed.author) {
        if (embed.author.name)
            result.header.author.text = embed.author.name

        if (embed.author.url)
            result.header.author.url = embed.author.url

        if (embed.author.icon_url)
            result.header.author.image_url = embed.author.icon_url
    }

    if (embed.description)
        result.body.description = embed.description

    if (embed.image)
        result.body.image_url = embed.image

    if (embed.title)
        result.body.title.text = embed.title

    if (embed.url)
        result.body.title.url = embed.url
    ...
    if (embed.footer) {
        ...
        if (embed.footer.icon_url)
            result.footer.image_url = embed.footer.icon_url
    }
